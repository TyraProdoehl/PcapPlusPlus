...
public:
	 ...
	 */
	typedef void (*OnTcpPacketReady)(int side, const ConnectionData& connData, const SPRawPacket& spRawPacket, void* userCookie);
	 ...
	 * @param[in] userCookie A pointer to the cookie provided by the user in TcpSorter c'tor (or nullptr if no cookie provided)
	 */
	typedef void (*OnTcpPacketMissing)(int side, const ConnectionData& connData, uint32_t seq, uint32_t length, void* userCookie);

	/**
	 * @typedef onTcpConnectionClosed
	 * A callback invoked when TCP connection is closed
	 * @param[in] connData Connection meta data
	 * @param[in] userCookie A pointer to the cookie provided by the user in TcpSorter c'tor (or nullptr if no cookie provided)
	 */
	typedef void (*OnTcpConnectionClosed)(const ConnectionData& connData, void* userCookie);
	 ...
	 * @param[in] onConnectionClosedCallback The callback to be invoked when TCP connection is closed
	...
	TcpSorter(OnTcpPacketReady onPacketReadyCallback,
				 ...
				 OnTcpConnectionClosed onConnectionClosedCallback = nullptr,
	 ...
	 */
	void sortPacket(const SPRawPacket& spRawPacket);
...
private:
	...
	OnTcpConnectionClosed m_OnConnectionClosedCallBack;
	 ...
	 */
	void flushPacket(const SPTcpSorterData& tcpSorterData, uint32_t ack, int rcvIdx);
};
