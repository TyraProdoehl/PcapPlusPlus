...
TcpSorter::TcpSorter(
							...
							OnTcpConnectionClosed onConnectionClosedCallback,
							void* userCookie,
							const TcpSorterConfiguration &config)
{
	...
	m_OnConnectionClosedCallBack = onConnectionClosedCallback;
...
void TcpSorter::sortPacket(const SPRawPacket& spRawPacket)
{
	...
	if (iter == m_ConnectionList.end())
	{
		...
		tcpSorterData->connData.srcPort = be16toh(tcpLayer->getTcpHeader()->portSrc);
		tcpSorterData->connData.dstPort = be16toh(tcpLayer->getTcpHeader()->portDst);
	...
	uint32_t seq = be32toh(tcpLayer->getTcpHeader()->sequenceNumber);
	uint32_t ack = be32toh(tcpLayer->getTcpHeader()->ackNumber);
...
void TcpSorter::closeConnection(uint32_t flowKey)
{
	...
	if (iter != m_ConnectionList.end())
	{
		...
		// invoke TCP connection closed call back
		if (nullptr != m_OnConnectionClosedCallBack)
		{
			m_OnConnectionClosedCallBack(tcpSorterData->connData, m_UserCookie);
		}
	}
}
 ...
 */
void TcpSorter::flushPacket(const SPTcpSorterData& tcpSorterData, uint32_t ack, int rcvIdx)
{
	int sndIdx = 1 - rcvIdx;
	...
	while (!aPacketList.empty())
	{
		...
		{
			if (nullptr != m_OnPacketMissingCallback)
			{
